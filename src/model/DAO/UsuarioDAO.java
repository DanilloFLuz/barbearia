...
public class UsuarioDAO {
    ...
    public boolean existeUsuario(Usuario autentica) throws SQLException {
        String sql = "SELECT * FROM autenticar "
                + "WHERE usuario = ? and senha = ? ";
        PreparedStatement statement = conexao.prepareStatement(sql);
        statement.setString(1, autentica.getNome());
        statement.setString(2, autentica.getSenha());
        statement.execute();
